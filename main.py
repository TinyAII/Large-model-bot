...
class Main(Star):
    ...
    async def text_to_image_menu_style(self, text: str) -> str:
        ...
        try:
            ...
            # 检测是否为解题助手内容
            is_solution = any(line.startswith('题目：') or line.startswith('思考过程：') or line.startswith('答案：') or line.startswith('时间：') for line in lines)
            ...
            for line in lines:
                ...
                if is_solution:
                    ...
                    if line.startswith('题目：'):
                        ...
                        html_parts.append('<div class="category-title">题目：</div>')
                        continue
                    elif line.startswith('思考过程：'):
                        current_section = "thinking"
                        html_parts.append('<div class="category-title">思考过程：</div>')
                        html_parts.append('<div class="thinking-section">')
                        continue
                    elif line.startswith('答案：'):
                        current_section = "answer"
                        html_parts.append('<div class="category-title">答案：</div>')
                        html_parts.append('<div class="answer-section">')
                        continue
                    elif line.startswith('时间：'):
                        current_section = "time"
                        html_parts.append('<div class="category-title">时间：</div>')
                        continue
            ...
            # 生成标题
            if is_solution:
                title_html = '<h1 class="menu-title">解题助手</h1>'
            else:
                title_html = '<h1 class="menu-title">大模型菜单</h1>'
            
            # 渲染HTML模板
            html_content = self.MENU_TEMPLATE.replace("{{title}}", title_html).replace("{{content}}", formatted_html)
